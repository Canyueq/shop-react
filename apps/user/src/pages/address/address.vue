...
import type { AddressListItem, AddressListRes, SetDefaultAddressRes, SetDefaultAddressReq } from "./index";
import { list, setDefault } from "../../common/api/address";

const userStore = useUserStore();

// 定义地址列表数据（指定精准类型）
const addressData = ref<AddressListItem[]>([]);

/**
 * 设为默认地址
 * @param item 地址项
 */
const handleDefault = async (item: AddressListItem) => {
  if (!item.id) {
 ...
 */
const handleAddressOpen = (address?: AddressListItem) => {
  // 传递当前编辑的地址到store
  if (address) {
...
const loadAddressList = async () => {
  try {
    // 调用列表接口并指定返回类型
    const res = (await list()) as { data: AddressListItem[] };
    // 赋值并确保是数组类型

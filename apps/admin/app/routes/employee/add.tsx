...
import type { AddEmployee } from "app/types/api/employee";

const EmployeeAdd = (param: EmployeeAddType) => {
  const { handleOk, handleCancel, records } = param;
  const [form] = Form.useForm();
  const onSave: FormProps<AddEmployee>["onFinish"] = async (
    values: AddEmployee,
  ) => {
    // console.log(values);
    const res: BaseRes<AxiosResponse> = await addEmployee(values);
    // console.log(res);
    switch (res.code) {
      case 1: {
